...
describe('Form Task Create - Input & Select Functionality', () => {
  it('should display the input & selected information', () => {
    cy.get('[data-testid="form-task-create-title"]')
      .type('Test Task Title')
      .should('have.value', 'Test Task Title');
    cy.get('[data-testid="form-task-create-description"]')
      .type('Test Task Description')
      .should('have.value', 'Test Task Description');
    ...
    cy.get('[data-testid="form-task-create-status"]').should(
      'contain',
      'In Progress'
    );
    ...
    cy.get('[data-testid="form-task-create-priority"]').should(
      'contain',
      'High'
    );
    ...
    cy.get('[data-testid="form-task-create-assignee"]').should(
      'contain',
      'Member 1'
    );
  });
...
describe('Form Task Create - Form Validation & Error Message', () => {
  it('should display the error message if missing title', () => {
    ...
    cy.get('[data-slot="form-label"')
      .contains('Title')
      .should('have.class', 'data-[error=true]:text-destructive-foreground');
    cy.get('[data-testid="form-task-create-title"]').should(
      'have.class',
      'aria-invalid:border-destructive'
    );
    cy.checkVisibilityOfText('Task title could not be empty.');
    cy.contains('Task title could not be empty.').should(
      'have.class',
...
describe('Form Task Create - Form Submission', () => {
  it('should create the task with correct details on form submission', () => {
    ...
    cy.get('[data-testid="form-task-create-description"]').type(
    ...
    cy.get('[data-testid="task-list-row"]')
      .first()
      .invoke('text')
      .should('contain', 'Test Task Title');
    cy.get('[data-testid="task-list-row"]')
      .first()
      .invoke('text')
